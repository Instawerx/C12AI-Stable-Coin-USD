...
exports.manualPaymentFunctions = exports.getAnalytics = exports.listPayments = exports.getManualPayment = exports.verifyManualPayment = exports.submitPaymentProof = exports.createManualPayment = void 0;
 ...
 */
exports.createManualPayment = functions.https.onCall(async (data, context) => {
    var _a, _b, _c;
    try {
        ...
        const { referenceId, tokenType, requestedAmount, tokenAmount, deliveryChain, paymentMethod, paymentAmount, cashAppCashtag, cashAppProof, stablecoinType, senderAddress, txHash, blockchainChain, userNotes, } = data;
        // Validation
        if (!referenceId || !tokenType || !requestedAmount || !deliveryChain || !paymentMethod) {
            throw new functions.https.HttpsError('invalid-argument', 'Missing required fields');
        }
        // Validate amounts
        const minPurchase = parseFloat(((_a = (await prisma.systemConfig.findUnique({ where: { key: 'MIN_PURCHASE_USD' } }))) === null || _a === void 0 ? void 0 : _a.value) || '10.00');
        const maxPurchase = parseFloat(((_b = (await prisma.systemConfig.findUnique({ where: { key: 'MAX_PURCHASE_USD' } }))) === null || _b === void 0 ? void 0 : _b.value) || '50000.00');
        if (parseFloat(requestedAmount) < minPurchase) {
            throw new functions.https.HttpsError('invalid-argument', `Minimum purchase is $${minPurchase}`);
        }
        if (parseFloat(requestedAmount) > maxPurchase) {
            throw new functions.https.HttpsError('invalid-argument', `Maximum purchase is $${maxPurchase} without enhanced KYC`);
        }
        ...
        const expiryHours = parseInt(((_c = (await prisma.systemConfig.findUnique({ where: { key: 'MANUAL_PAYMENT_EXPIRY_HOURS' } }))) === null || _c === void 0 ? void 0 : _c.value) || '24');
...
exports.submitPaymentProof = functions.https.onCall(async (data, context) => {
    try {
        ...
        if (payment.status !== 'PENDING_SUBMISSION') {
            throw new functions.https.HttpsError('failed-precondition', `Payment already ${payment.status}`);
        }
        // Check expiry
        ...
        if (payment.paymentMethod === 'CASH_APP') {
            if (!cashAppProof || !senderInfo) {
                throw new functions.https.HttpsError('invalid-argument', 'Cash App proof and cashtag required');
            }
            ...
            if (!txHash || !senderInfo) {
                throw new functions.https.HttpsError('invalid-argument', 'Transaction hash and wallet address required');
            }
        ...
        // Notify user
        await (0, notifications_1.sendNotification)(user.id, {
...
exports.verifyManualPayment = functions.https.onCall(async (data, context) => {
    try {
        ...
        const hasFinanceRole = adminUser.adminRoles.some((role) => role.role === 'FINANCE_ADMIN' || role.role === 'SUPER_ADMIN');
        if (!hasFinanceRole) {
        ...
        if (payment.status !== 'PENDING_VERIFICATION') {
            throw new functions.https.HttpsError('failed-precondition', `Payment status is ${payment.status}, cannot verify`);
        }
        if (approved) {
            ...
            // Notify user
            await (0, notifications_1.sendNotification)(payment.userId, {
 ...
 */
async function distributeTokens(paymentId, payment) {
    try {
 ...
 */
async function mintC12USD(payment) {
 ...
 */
async function transferC12DAO(payment) {
 ...
 */
exports.getManualPayment = functions.https.onCall(async (data, context) => {
    var _a;
    try {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'Authentication required');
        ...
        return {
            ...
            payment: {
                ...
                distributedAt: (_a = payment.distributedAt) === null || _a === void 0 ? void 0 : _a.toISOString(),
    ...
    try {
        ...
        const formattedPayments = payments.map((payment) => {
            ...
            return ({
                ...
                verifiedAt: (_b = payment.verifiedAt) === null || _b === void 0 ? void 0 : _b.toISOString(),
            });
        });
        return {
            success: true,
...
exports.getAnalytics = functions.https.onCall(async (data, context) => {
    try {
        ...
        const totalVolume = payments.reduce((sum, p) => sum + parseFloat(p.requestedAmount.toString()), 0);
        const completedPayments = payments.filter((p) => p.status === 'COMPLETED');
        const completedVolume = completedPayments.reduce((sum, p) => sum + parseFloat(p.requestedAmount.toString()), 0);
        const pendingPayments = payments.filter((p) => p.status === 'PENDING_VERIFICATION' || p.status === 'PENDING_SUBMISSION');
        const pendingVolume = pendingPayments.reduce((sum, p) => sum + parseFloat(p.requestedAmount.toString()), 0);
        const rejectedPayments = payments.filter((p) => p.status === 'REJECTED');
        const averageAmount = totalPayments > 0 ? totalVolume / totalPayments : 0;
        // Calculate average verification time (in hours)
        const verifiedPayments = payments.filter((p) => p.verifiedAt && p.createdAt);
        const totalVerificationTime = verifiedPayments.reduce((sum, p) => {
            const diff = p.verifiedAt.getTime() - p.createdAt.getTime();
            return sum + diff / (1000 * 60 * 60); // Convert to hours
        }, 0);
        const averageVerificationTime = verifiedPayments.length > 0
...
exports.manualPaymentFunctions = {
    createManualPayment: exports.createManualPayment,
    submitPaymentProof: exports.submitPaymentProof,
    verifyManualPayment: exports.verifyManualPayment,
    getManualPayment: exports.getManualPayment,
